...
type MobileFinancePrefs={monthlyBudget?:number};
type MobileStoreData=StoreData&{mobileFinance?:MobileFinancePrefs};
...
function financePrefs(data:StoreData){return (data as MobileStoreData).mobileFinance||{}}
...
export default function MobileCollector({status,profileName,data,update,config,session}:Props){
  ...
  const moveToWishlist=(itemId:string)=>patchData(d=>({...d,items:d.items.map(i=>{if(i.id!==itemId)return i;if(i.status==='wishlist'&&i.customFields?.__wishlist!=='true')return i;const cf={...(i.customFields||{})};delete cf.__wishlist;return {...i,status:'wishlist',quantity:Math.max(1,i.quantity||1),customFields:cf,updatedAt:nowIso()}})}));
  ...
  if(detail){
    return <div className="mobile-collector-shell mobile-app-active">
      ...
      <main className="mc-screen-wrap mc-detail-scroll">
        <MobileProductDetail item={detail} data={data} update={update} close={()=>setDetailId(null)} onAdd={()=>addOne(detail.id)} onSetQty={(n)=>setOwnedQty(detail.id,n)} onWishlist={()=>moveToWishlist(detail.id)} openEdit={()=>openEdit({kind:'product',itemId:detail.id})} notify={notify}/>
      </main>
  ...
  return <div className="mobile-collector-shell mobile-app-active">
    ...
    <main className="mc-screen-wrap">
      {view==='home'&&<MobileHome data={data} update={update} notify={notify} items={portfolioItems} value={portfolioValue} tab={homeTab} setTab={setHomeTab} portfolioName={portfolioName} portfolioId={portfolioId} choosePortfolio={()=>setSheet('portfolio')}/>} 
...
function MobileHome({data,update,notify,items,value,tab,setTab,portfolioName,portfolioId,choosePortfolio}:{data:StoreData;update:(s:StoreData)=>void;notify:(s:string)=>void;items:Item[];value:number;tab:'overview'|'performance';setTab:(t:'overview'|'performance')=>void;portfolioName:string;portfolioId:string;choosePortfolio:()=>void}){
  const [range,setRange]=useState<RangeKey>('1M');
  const finance=financePrefs(data),[budgetText,setBudgetText]=useState(String(Number(finance.monthlyBudget)||0));
  const now=new Date(),monthKey=`${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}`;
  const monthlySpend=data.items.filter(i=>i.status!=='wishlist'&&i.purchaseDate?.slice(0,7)===monthKey).reduce((sum,i)=>sum+(Number(i.purchasePrice)||0)*Math.max(1,i.quantity||1),0);
  const budget=Math.max(0,Number(budgetText)||0),hasBudget=budget>0,remaining=hasBudget?budget-monthlySpend:0,budgetPct=hasBudget?Math.min(100,monthlySpend/budget*100):0;
  const costBasis=items.reduce((sum,i)=>sum+(Number(i.purchasePrice)||0)*Math.max(1,i.quantity||1),0),marketGain=value-costBasis;
  const delta=portfolioDelta(data,portfolioId,value,30);
  const most=[...items].sort((a,b)=>holdingValue(b)-holdingValue(a)).slice(0,10);
  const saveBudget=()=>{const n=Math.max(0,Number(budgetText)||0);update({...data,mobileFinance:{...finance,monthlyBudget:n}} as StoreData);setBudgetText(String(n));notify('Monthly collecting budget saved')};
  return <section className="mc-page mc-home">
    <div className="mc-home-tabs"><button className={tab==='overview'?'active':''} onClick={()=>setTab('overview')}>Overview</button><button className={tab==='performance'?'active':''} onClick={()=>setTab('performance')}>Performance</button><button className="mc-currency-dot" onClick={()=>setTab('performance')} aria-label="Open collecting budget">$</button><span className="mc-currency-label">USD</span></div>
    {tab==='overview'?<>
    ...
    </>:<div className="mc-performance-stack">
      <section className="mc-budget-card">
        <header><div><CircleDollarSign/><span><b>Monthly Collecting Budget</b><small>{new Intl.DateTimeFormat('en-US',{month:'long',year:'numeric'}).format(now)}</small></span></div><button className="mc-plaid-connect" onClick={()=>notify('Plaid needs your API credentials before bank linking can be turned on')}>Connect Bank</button></header>
        <div className="mc-budget-numbers"><span><small>Spent</small><b>{money(monthlySpend)}</b></span><span><small>{!hasBudget?'Budget not set':remaining>=0?'Remaining':'Over budget'}</small><b className={!hasBudget?'':remaining>=0?'gain':'loss'}>{hasBudget?money(Math.abs(remaining)):'—'}</b></span></div>
        <div className="mc-budget-bar"><i style={{width:`${budgetPct}%`}}/></div>
        <label className="mc-budget-input"><span>Monthly budget</span><div><b>$</b><input inputMode="decimal" value={budgetText} onChange={e=>setBudgetText(e.target.value.replace(/[^0-9.]/g,''))}/><button onClick={saveBudget}>Save</button></div></label>
        <p>For now, spend is calculated from the Price Paid and Purchase Date saved on your collectibles. Plaid can feed verified bank transactions into this same budget view once connected.</p>
      </section>
      <section className="mc-performance-card mc-market-gain-card"><div className="mc-performance-title"><span><small>Market Gain</small><strong className={marketGain>=0?'gain':'loss'}>{marketGain>=0?'+':''}{money(marketGain)}</strong></span><span><small>Cost Basis</small><b>{money(costBasis)}</b></span></div><MarketGainChart data={data} currentValue={value} costBasis={costBasis} range={range}/><RangeRow range={range} setRange={setRange}/><p>Market gain compares your current collection value against what you paid. Negative values are shown below zero.</p></section>
    </div>}
  </section>;
}

function MarketGainChart({data,currentValue,costBasis,range}:{data:StoreData;currentValue:number;costBasis:number;range:RangeKey}){
  const id=useId().replace(/:/g,''),pts=chartPoints(data,'all',currentValue,range).map(p=>({...p,value:p.value-costBasis})),w=390,h=150,pad=9,values=pts.map(x=>x.value),min=Math.min(...values,0),max=Math.max(...values,0),span=Math.max(1,max-min),x=(i:number)=>pts.length===1?w/2:(i/(pts.length-1))*w,y=(v:number)=>pad+(1-(v-min)/span)*(h-pad*2),line=pts.map((pt,i)=>`${i?'L':'M'}${x(i).toFixed(1)} ${y(pt.value).toFixed(1)}`).join(' '),zero=y(0),positive=pts.at(-1)!.value>=0,color=positive?'#22c58b':'#ff464e';
  return <div className="mc-market-gain-chart"><svg viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none"><defs><linearGradient id={id} x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor={color} stopOpacity=".28"/><stop offset="1" stopColor={color} stopOpacity="0"/></linearGradient></defs><line x1="0" x2={w} y1={zero} y2={zero} stroke="#35353a" strokeWidth="1" strokeDasharray="4 5"/><path d={`${line} L${w} ${h} L0 ${h} Z`} fill={`url(#${id})`}/><path d={line} fill="none" stroke={color} strokeWidth="2.6" strokeLinecap="round" strokeLinejoin="round"/></svg></div>;
}
...
function MobileSearchFlow({data,update,categories,sets,screen,setScreen,query,setQuery,selectedCategory,setSelectedCategory,selectedSet,setSelectedSet,openCard,openFilters,openSort,openProgress,editMode,toggleEdit,openEdit,addCategory,addCollection,addToCollection,onAdd}:{data:StoreData;update:(s:StoreData)=>void;categories:CategoryGroup[];sets:SetGroup[];screen:SearchScreen;setScreen:(s:SearchScreen)=>void;query:string;setQuery:(v:string)=>void;selectedCategory:string;setSelectedCategory:(v:string)=>void;selectedSet:SetGroup|null;setSelectedSet:(s:SetGroup|null)=>void;openCard:(id:string)=>void;openFilters:()=>void;openSort:()=>void;openProgress:()=>void;editMode:boolean;toggleEdit:()=>void;openEdit:(t:EditTarget)=>void;addCategory:()=>void;addCollection:()=>void;addToCollection:()=>void;onAdd:(id:string)=>void}){
  if(screen==='root')return <section className="mc-page"><MobileTopSearch placeholder="Search for products" value={query} setValue={setQuery} onFilter={openFilters} onSort={openSort} editMode={editMode} onEdit={toggleEdit}/><div className="mc-quick-heading-row"><h3 className="mc-heading">Quick Filters</h3>{editMode&&<button className="mc-add-filter" onClick={addCategory}><Plus/>Add Filter</button>}</div><div className="mc-quick-grid">{categories.filter(c=>!query||c.name.toLowerCase().includes(query.toLowerCase())).map(c=><div key={c.key} className="mc-cover-card" role="button" tabIndex={0} style={coverStyle(c.cover)} aria-label={c.name} title={c.name} onClick={()=>{setSelectedCategory(c.rawName);setScreen('sets')}}><span className="mc-filter-name">{c.name}</span>{editMode&&<button className="mc-card-edit" onClick={e=>{e.stopPropagation();openEdit({kind:'category',key:c.key,rawName:c.rawName,name:c.name})}}><Pencil/></button>}</div>)}</div></section>;
  if(screen==='sets')return <MobileSets data={data} sets={sets} query={query} setQuery={setQuery} back={()=>setScreen('root')} openSet={(s)=>{setSelectedSet(s);setScreen('set')}} openFilters={openFilters} editMode={editMode} toggleEdit={toggleEdit} openEdit={openEdit} addSet={addCollection}/>;
  return <MobileSetProducts data={data} update={update} group={selectedSet} query={query} setQuery={setQuery} back={()=>setScreen('sets')} openCard={openCard} openFilters={openFilters} openSort={openSort} editMode={editMode} toggleEdit={toggleEdit} openEdit={openEdit} addToCollection={addToCollection} onAdd={onAdd} openProgress={openProgress}/>;
}

function MobileSets({data,sets,query,setQuery,back,openSet,openFilters,editMode,toggleEdit,openEdit,addSet}:{data:StoreData;sets:SetGroup[];query:string;setQuery:(v:string)=>void;back:()=>void;openSet:(s:SetGroup)=>void;openFilters:()=>void;editMode:boolean;toggleEdit:()=>void;openEdit:(t:EditTarget)=>void;addSet:()=>void}){
  const shown=sets.filter(s=>!query||s.name.toLowerCase().includes(query.toLowerCase()));
  return <section className="mc-page mc-sets"><div className="mc-title-row"><button className="mc-circle" onClick={back}><ArrowLeft/></button><h2>Sets</h2><span/></div><MobileTopSearch placeholder="Search sets" value={query} setValue={setQuery} onFilter={openFilters} editMode={editMode} onEdit={toggleEdit}/>{editMode&&<div className="mc-add-under-edit"><button className="mc-add-filter" onClick={addSet}><Plus/>Add Set</button></div>}<div className="mc-set-grid">{shown.map(s=>{const progress=getProgress(data,s);return <div className="mc-set-card mc-cover-card" role="button" tabIndex={0} style={coverStyle(s.cover)} key={s.key} onClick={()=>openSet(s)}><div className="mc-set-logo">{s.name.toUpperCase()}</div>{progress.enabled?<><small>Progress: {progress.owned}/{progress.target} ({progress.percent}%)</small><div className="mc-set-card-progress"><i style={{width:`${progress.percent}%`}}/></div></>:<small>{s.items.reduce((n,i)=>n+ownedQty(i),0)} items</small>}<small>Total Value: {money(s.items.reduce((n,i)=>n+holdingValue(i),0))}</small>{editMode&&<button className="mc-card-edit" onClick={e=>{e.stopPropagation();openEdit({kind:'collection',key:s.key,rawName:s.rawName,name:s.name,collectionId:s.collectionId})}}><Pencil/></button>}</div>})}</div></section>;
}

function MobileSetProducts({data,update,group,query,setQuery,back,openCard,openFilters,openSort,editMode,toggleEdit,openEdit,addToCollection,onAdd,openProgress}:{data:StoreData;update:(s:StoreData)=>void;group:SetGroup|null;query:string;setQuery:(v:string)=>void;back:()=>void;openCard:(id:string)=>void;openFilters:()=>void;openSort:()=>void;editMode:boolean;toggleEdit:()=>void;openEdit:(t:EditTarget)=>void;addToCollection:()=>void;onAdd:(id:string)=>void;openProgress:()=>void}){
...
function MobileProductDetail({item,data,update,close,onAdd,onSetQty,onWishlist,openEdit,notify}:{item:Item;data:StoreData;update:(s:StoreData)=>void;close:()=>void;onAdd:()=>void;onSetQty:(n:number)=>void;onWishlist:()=>void;openEdit:()=>void;notify:(s:string)=>void}){
  const [grade,setGrade]=useState<'RAW'|'GRADED'|'POP'>('RAW');
  const [range,setRange]=useState<RangeKey>('1M');
  const [marketBusy,setMarketBusy]=useState(false);
  const [marketMessage,setMarketMessage]=useState('');
  const ch=itemChange(item),qty=ownedQty(item),wishlisted=isWishlisted(item);
  ...
  const sendToWishlist=()=>{if(item.status==='wishlist'&&item.customFields?.__wishlist!=='true'){notify('Already wishlist only');return}onWishlist();notify('Moved to Wishlist')};
  return <section className="mc-page mc-product-detail">
    <div className="mc-detail-top"><button className="mc-circle mc-detail-control mc-detail-back" onClick={close} aria-label="Back"><ArrowLeft/></button><div className="mc-detail-right-controls"><button className={`mc-circle mc-detail-control mc-detail-wishlist ${wishlisted?'active':''}`} onClick={sendToWishlist} aria-label="Move to wishlist"><Star fill={wishlisted?'currentColor':'none'}/></button><button className="mc-circle mc-detail-control mc-detail-edit-button" onClick={openEdit} aria-label="Edit product"><Pencil/></button></div></div>
