...
export default function MobileCollector({status,profileName,data,update}:Props){
  ...
  if(detail){
    return <div className="mobile-collector-shell mobile-app-active">
      <MobileProductDetail item={detail} data={data} update={update} close={()=>setDetailId(null)} onAdd={()=>addOne(detail.id)} onSetQty={(n)=>setOwnedQty(detail.id,n)} editMode={editMode} toggleEdit={()=>setEditMode(v=>!v)} openEdit={()=>openEdit({kind:'product',itemId:detail.id})} notify={notify}/>
  ...
  return <div className="mobile-collector-shell mobile-app-active">
    <main className="mc-screen-wrap">
      ...
      {view==='search'&&<MobileSearchFlow data={data} update={update} categories={categories} sets={setGroups} screen={searchScreen} setScreen={setSearchScreen} query={query} setQuery={setQuery} selectedCategory={selectedCategory} setSelectedCategory={setSelectedCategory} selectedSet={activeSet} setSelectedSet={setSelectedSet} openCard={setDetailId} openFilters={()=>setSheet('filters')} openSort={()=>setSheet('sort')} openProgress={()=>setSheet('progress')} editMode={editMode} toggleEdit={()=>setEditMode(v=>!v)} openEdit={openEdit} addCategory={()=>openCreate('category')} addCollection={()=>openCreate('collection')} addProduct={()=>openCreate('product')} addToCollection={()=>{setPickerSelected([]);setSheet('picker')}} onAdd={addOne}/>} 
...
function MobileSearchFlow({data,update,categories,sets,screen,setScreen,query,setQuery,selectedCategory,setSelectedCategory,selectedSet,setSelectedSet,openCard,openFilters,openSort,openProgress,editMode,toggleEdit,openEdit,addCategory,addCollection,addProduct,addToCollection,onAdd}:{data:StoreData;update:(s:StoreData)=>void;categories:CategoryGroup[];sets:SetGroup[];screen:SearchScreen;setScreen:(s:SearchScreen)=>void;query:string;setQuery:(v:string)=>void;selectedCategory:string;setSelectedCategory:(v:string)=>void;selectedSet:SetGroup|null;setSelectedSet:(s:SetGroup|null)=>void;openCard:(id:string)=>void;openFilters:()=>void;openSort:()=>void;openProgress:()=>void;editMode:boolean;toggleEdit:()=>void;openEdit:(t:EditTarget)=>void;addCategory:()=>void;addCollection:()=>void;addProduct:()=>void;addToCollection:()=>void;onAdd:(id:string)=>void}){
  ...
  return <MobileSetProducts data={data} update={update} group={selectedSet} query={query} setQuery={setQuery} back={()=>setScreen('sets')} openCard={openCard} openFilters={openFilters} openSort={openSort} editMode={editMode} toggleEdit={toggleEdit} openEdit={openEdit} addAnother={addProduct} addToCollection={addToCollection} onAdd={onAdd} openProgress={openProgress}/>;
}
...
function MobileSetProducts({data,update,group,query,setQuery,back,openCard,openFilters,openSort,editMode,toggleEdit,openEdit,addAnother,addToCollection,onAdd,openProgress}:{data:StoreData;update:(s:StoreData)=>void;group:SetGroup|null;query:string;setQuery:(v:string)=>void;back:()=>void;openCard:(id:string)=>void;openFilters:()=>void;openSort:()=>void;editMode:boolean;toggleEdit:()=>void;openEdit:(t:EditTarget)=>void;addAnother:()=>void;addToCollection:()=>void;onAdd:(id:string)=>void;openProgress:()=>void}){
...
function MobileProductDetail({item,data,update,close,onAdd,onSetQty,editMode,toggleEdit,openEdit,notify}:{item:Item;data:StoreData;update:(s:StoreData)=>void;close:()=>void;onAdd:()=>void;onSetQty:(n:number)=>void;editMode:boolean;toggleEdit:()=>void;openEdit:()=>void;notify:(s:string)=>void}){
  ...
  return <section className="mc-page mc-product-detail">
    <div className="mc-detail-top"><button className="mc-circle" onClick={close}><ArrowLeft/></button><EditToggle active={editMode} onClick={toggleEdit}/><button className="mc-circle"><Share2/></button></div>
    {editMode&&<div className="mc-detail-edit-row"><button onClick={openEdit}><Pencil/>Quick edit product</button></div>}
