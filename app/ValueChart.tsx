...
export default function ValueChart({data,collectionId,value,cost,scopeId,scopeOptions,onScopeChange}:{data:Store;collectionId:string;value:number;cost:number;scopeId?:string;scopeOptions?:ScopeOption[];onScopeChange?:(id:string)=>void}){
  ...
  return <section className="value-chart">
    ...
    <div className="chart-canvas"><svg viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" role="img" aria-label="Collection value history" onMouseLeave={()=>setHover(null)}>
      <defs><linearGradient id={`red-area-${collectionId}`} x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="var(--accent)" stopOpacity=".24"/><stop offset="100%" stopColor="var(--accent)" stopOpacity="0"/></linearGradient></defs>
