...
export default function VexumSocial(){
  ...
  const productOptions=useMemo(()=>{
    ...
    for(const item of workspace.data.items.filter(item=>item.status==='owned')){
      ...
      byKey.set(key,{
        ...
        tag:{
          product_id:null,portfolio_item_id:item.id,tag_type:'owned_copy',
          product_snapshot:{
  ...
  if(!workspace.ready||loading)return <div className="vxsoc-page"><div className="vxsoc-loading">Loading collector network…</div></div>;
...
function PostComposer({profile,communities,products,setups,collections,onClose,onSave,busy}:{
  ...
  return <div className="vxsoc-modal-backdrop" onMouseDown={e=>{if(e.currentTarget===e.target)onClose()}}><section className="vxsoc-modal vxsoc-composer">
    <header><div><Avatar profile={profile}/><span><strong>Create Post</strong><small>Structured collector context stays attached to the post.</small></span></div><button onClick={onClose}><X/></button></header>
...
function CommunityComposer({onClose,onSave,busy}:{onClose:()=>void;onSave:(input:{name:string;description:string;visibility:'public'|'private'})=>void;busy:boolean}){
  ...
  return <div className="vxsoc-modal-backdrop"><section className="vxsoc-modal small"><header><div><Users/><span><strong>Create Community</strong><small>Start with a product-aware feed. Channels and realtime chat can layer on later.</small></span></div><button onClick={onClose}><X/></button></header><div className="vxsoc-form"><label>Name<input value={name} onChange={e=>setName(e.target.value)} placeholder="Marvel Legends"/></label><label>Description<textarea value={description} onChange={e=>setDescription(e.target.value)} placeholder="Collectors, releases, setups, trades, and product knowledge."/></label><label>Visibility<select value={visibility} onChange={e=>setVisibility(e.target.value as 'public'|'private')}><option value="public">Public</option><option value="private">Private</option></select></label></div><footer><button onClick={onClose}>Cancel</button><button className="red" disabled={busy||name.trim().length<3} onClick={()=>onSave({name,description,visibility})}>Create</button></footer></section></div>;
}
