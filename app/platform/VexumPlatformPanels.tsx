...
export function NotificationCenter({open,onClose,workspace,navigate}:{open:boolean;onClose:()=>void;workspace:Workspace;navigate:Navigate}){
  ...
  return <div className="vxp-flyout notification" role="region" aria-label="Notifications"><header><div><Bell/><span><strong>Notifications</strong><small>{notifications.filter(n=>!read.has(n.id)).length} unread</small></span></div><button aria-label="Close notifications" onClick={onClose}><X/></button></header><nav aria-label="Notification categories">{(['All','Tasks','Radar','Sell','Financial','Social'] as const).map(name=><button aria-pressed={tab===name} className={tab===name?'active':''} onClick={()=>setTab(name as any)} key={name}>{name}</button>)}</nav>
