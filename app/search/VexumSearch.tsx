...
const WISHLIST_META_KEY='vexum.wishlist.meta.v1';
...
function money(n?:number){return typeof n==='number'?'
...
function WishlistModal({product,relation,onClose,onSave}:{product:NormalizedProduct;relation:UserProductRelationship;onClose:()=>void;onSave:(r:UserProductRelationship)=>void}){
  const saved=useMemo(()=>loadWishlistPhaseMeta(product.id),[product.id]);
  const [priority,setPriority]=useState(saved?.priority||(relation.grail?'Grail':'Medium'));
  const [target,setTarget]=useState(relation.targetPrice?String(relation.targetPrice):saved?.targetPrice?String(saved.targetPrice):product.msrp?String(product.msrp):'');
  const [max,setMax]=useState(relation.maxPrice?String(relation.maxPrice):saved?.maxPrice?String(saved.maxPrice):'');
  const [condition,setCondition]=useState(relation.conditionRequirement||saved?.desiredCondition||'Any');
  const [msrpAlert,setMsrpAlert]=useState(saved?.alerts?.msrp??true);
  const [localAlert,setLocalAlert]=useState(saved?.alerts?.local??false);
  const [marketAlert,setMarketAlert]=useState(saved?.alerts?.marketplace??true);
  return <div className="vxs-modal-backdrop" role="dialog" aria-modal="true"><div className="vxs-quick-modal">
    ...
    <footer><button onClick={onClose}>Cancel</button><button className="primary" onClick={()=>{
      const targetPrice=Number(target)||undefined;
      const maxPrice=Number(max)||undefined;
      saveWishlistPhaseMeta(product.id,{priority,targetPrice,maxPrice,condition,msrpAlert,localAlert,marketAlert});
      onSave({...relation,wishlisted:true,grail:priority==='Grail',targetPrice,maxPrice,conditionRequirement:condition,tracked:msrpAlert||localAlert||marketAlert});
    }}><Star/>Save Wishlist</button></footer>
  </div></div>;
}
...
+n.toFixed(2):'—'}
function loadWishlistPhaseMeta(productId:string){
  if(typeof window==='undefined')return null;
  try{
    const saved=JSON.parse(localStorage.getItem(WISHLIST_META_KEY)||'{}');
    return saved&&typeof saved==='object'?saved[productId]||null:null;
  }catch{return null}
}
function saveWishlistPhaseMeta(productId:string,value:{
  priority:string;targetPrice?:number;maxPrice?:number;condition:string;
  msrpAlert:boolean;localAlert:boolean;marketAlert:boolean;
}){
  try{
    const saved=JSON.parse(localStorage.getItem(WISHLIST_META_KEY)||'{}');
    const all=saved&&typeof saved==='object'?saved:{};
    const previous=all[productId]&&typeof all[productId]==='object'?all[productId]:{};
    const priorAlerts=previous.alerts&&typeof previous.alerts==='object'?previous.alerts:{};
    ...
    all[productId]={
      ...
      desiredCondition:value.condition,
      retailers:Array.isArray(previous.retailers)?previous.retailers:[],
      quantity:Math.max(1,Number(previous.quantity)||1),
      notes:typeof previous.notes==='string'?previous.notes:'',
      alerts:{
        price:value.targetPrice!==undefined,
        msrp:value.msrpAlert,
        restock:Boolean(priorAlerts.restock),
        local:value.localAlert,
        marketplace:value.marketAlert,
        release:Boolean(priorAlerts.release),
        preorder:Boolean(priorAlerts.preorder)
      },
      archived:false,
      archiveReason:undefined,
      history:[{at:new Date().toISOString(),event:'Wishlist preferences saved from Search'},...history].slice(0,80)
    };
    localStorage.setItem(WISHLIST_META_KEY,JSON.stringify(all));
  }catch{}
}
