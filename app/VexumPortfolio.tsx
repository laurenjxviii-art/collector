...
function PortfolioMetric({label,value,sub,tone='muted',trend,trendLabel='Recorded history'}:{label:string;value:string;sub:string;tone?:'muted'|'green'|'red'|'orange';trend?:number[];trendLabel?:string}){
  return <section className="vx-panel vxp2-metric"><span>{label}</span><strong>{value}</strong><small className={'tone-'+tone}>{sub}</small>{trend!==undefined?<VexumMetricTrend values={trend} label={trendLabel} negative={tone==='red'}/>:null}</section>;
}
...
function PortfolioItems({rows,store,view,columns,selectedIds,onSelect,onOpen,onFavorite}:{rows:Item[];store:Store;view:PortfolioViewMode;columns:string[];selectedIds:Set<string>;onSelect:(id:string)=>void;onOpen:(id:string)=>void;onFavorite:(item:Item)=>void}){
  ...
  return <div className={'vxp2-items-view '+view}>{rows.map(item=><article className={'vx-panel '+(selectedIds.has(item.id)?'selected':'')} key={item.id}><button className="select" onClick={()=>onSelect(item.id)}>{selectedIds.has(item.id)?<Check/>:<span/>}</button><button className="favorite" onClick={()=>onFavorite(item)}>{item.favorite?<Star fill="currentColor"/>:<Star/>}</button><button className="body" onClick={()=>onOpen(item.id)}><ItemThumb item={item} large={view==='gallery'}/><div className="copy"><small>{pathLabel(item,store.collections)}</small><strong>{item.name}</strong><span>{item.category} · {item.condition||'Condition missing'}</span>{view!=='compact'?<footer><div><small>Market</small><b>{portfolioMoney(item.currentValue*item.quantity)}</b></div><div><small>P/L</small><b className={item.currentValue>=item.purchasePrice?'tone-green':'tone-red'}>{item.currentValue>=item.purchasePrice?'+':''}{portfolioMoney((item.currentValue-item.purchasePrice)*item.quantity)}</b></div><div><small>Setup</small><b>{itemLocation(store,item)}</b></div></footer>:null}</div></button></article>)}</div>;
}
...
function PortfolioAnalytics({store,analytics,issues,onCategory}:{store:Store;analytics:ReturnType<typeof computePortfolioAnalytics>;issues:AuditIssue[];onCategory:(category:string)=>void}){
  ...
  const snapshots=(store.history||[]).filter(row=>Number.isFinite(Date.parse(row.date))).toSorted((a,b)=>a.date.localeCompare(b.date)).slice(-40);
  ...
  return <div className="vxp2-analytics">
    <div className="vxp2-metrics analytics"><PortfolioMetric label="Current Value" value={portfolioMoney(analytics.value)} sub="Owned inventory"/><PortfolioMetric label="Cost Basis" value={portfolioMoney(analytics.cost)} sub="Recorded cost"/><PortfolioMetric trend={[analytics.cost,analytics.value]} trendLabel="Cost → current value" label="Unrealized P/L" value={(analytics.unrealized>=0?'+':'')+portfolioMoney(analytics.unrealized)} sub={analytics.cost?(analytics.unrealized>=0?'+':'')+(analytics.unrealized/analytics.cost*100).toFixed(1)+'%':'—'} tone={analytics.unrealized>=0?'green':'red'}/><PortfolioMetric trend={analytics.sold.length?[analytics.sold.reduce((sum,item)=>sum+item.purchasePrice*item.quantity,0),analytics.sold.reduce((sum,item)=>sum+item.currentValue*item.quantity,0)]:[]} trendLabel="Sold cost → recorded proceeds" label="Realized Profit" value={(analytics.realized>=0?'+':'')+portfolioMoney(analytics.realized)} sub={analytics.sold.length+' sold records'} tone={analytics.realized>=0?'green':'red'}/><PortfolioMetric label="Items Owned" value={String(analytics.count)} sub={analytics.owned.length+' records'}/><PortfolioMetric label="Average Item Value" value={analytics.count?portfolioMoney(analytics.averageItemValue):'—'} sub="By owned quantity"/><PortfolioMetric label="Average Purchase Price" value={analytics.count?portfolioMoney(analytics.averagePurchasePrice):'—'} sub="By owned quantity"/><PortfolioMetric trend={analytics.msrpSavings?[0,analytics.msrpSavings]:[]} trendLabel="Savings against MSRP" label="MSRP Savings" value={analytics.msrpSavings?'+'+portfolioMoney(analytics.msrpSavings):'—'} sub="Where MSRP exists" tone="green"/><PortfolioMetric trend={analytics.paidOverMsrp?[0,-analytics.paidOverMsrp]:[]} trendLabel="Extra cost against MSRP" label="Paid Over MSRP" value={analytics.paidOverMsrp?'-'+portfolioMoney(analytics.paidOverMsrp):'—'} sub="Where MSRP exists" tone="red"/><PortfolioMetric label="Completion Rate" value={analytics.completionRate?analytics.completionRate.toFixed(1)+'%':'—'} sub={analytics.completedCollections+' completed collections'}/><PortfolioMetric label="Sealed / Open" value={analytics.sealed+' / '+analytics.open} sub="By item quantity"/><PortfolioMetric label="Missing Receipts" value={String(missingReceipts)} sub={duplicates+' duplicate groups'} tone={missingReceipts?'orange':'green'}/></div>
    <div className="vxp2-analytics-grid"><section className="vx-panel"><header><div><h3>Portfolio Value Over Time</h3><p>Stored VEXUM snapshots.</p></div></header><SimpleLine values={history} dates={snapshots.map(row=>row.date)}/></section><section className="vx-panel"><header><div><h3>Value by Category</h3><p>Click to drill into All Items.</p></div></header><div className="vxp2-bars">{analytics.categories.map(row=><button key={row.name} onClick={()=>onCategory(row.name)}><span><strong>{row.name}</strong><b>{portfolioMoney(row.value)}</b></span><i><b style={{width:(analytics.value?row.value/analytics.value*100:0)+'%'}}/></i><small>{row.count} items</small></button>)}</div></section><section className="vx-panel"><header><h3>Collection Behavior</h3></header><div className="vxp2-analytic-rows"><div><span>Most collected brand</span><strong>{analytics.topBrand?.[0]||'—'}</strong><b>{analytics.topBrand?.[1]||0} items</b></div><div><span>Most used retailer</span><strong>{analytics.topRetailer?.[0]||'—'}</strong><b>{analytics.topRetailer?.[1]||0} purchases</b></div><div><span>Purchases / month</span><strong>{analytics.count?analytics.purchasesPerMonth.toFixed(1):'—'}</strong><b>observed months</b></div><div><span>Spend / month</span><strong>{analytics.cost?portfolioMoney(analytics.spendPerMonth):'—'}</strong><b>recorded cost</b></div></div></section><section className="vx-panel"><header><h3>Category P/L</h3></header><div className="vxp2-analytic-rows">{analytics.categories.map(row=><div key={row.name}><span>{row.name}</span><strong>{portfolioMoney(row.value)}</strong><b className={row.pl>=0?'tone-green':'tone-red'}>{row.pl>=0?'+':''}{portfolioMoney(row.pl)}</b></div>)}</div></section></div>
  </div>;
}
...
function BulkBar({count,collections,onMove,onTag,onField,onSetup,onSell,onArchive,onExport,onDelete,onClear}:{count:number;collections:Collection[];onMove:(id:string)=>void;onTag:()=>void;onField:()=>void;onSetup:()=>void;onSell:()=>void;onArchive:()=>void;onExport:()=>void;onDelete:()=>void;onClear:()=>void}){
  return <div className="vxp2-bulk"><strong>{count} selected</strong><select defaultValue="" onChange={e=>{if(e.target.value!=='__')onMove(e.target.value);e.currentTarget.value=''}}><option value="__">Move to collection…</option><option value="">All Items</option>{collections.map(collection=><option key={collection.id} value={collection.id}>{collectionPath(collection.id,collections).map(c=>c.name).join(' / ')}</option>)}</select><button onClick={onTag}><Tag/>Add Tag</button><button onClick={onField}><Settings2/>Change Field</button><button onClick={onSetup}><Layers3/>Assign Setup</button><button onClick={onSell}><ShoppingBag/>Sell</button><button onClick={onArchive}><Archive/>Archive</button><button onClick={onExport}><Download/>Export</button><button className="danger" onClick={onDelete}><Trash2/>Delete</button><button onClick={onClear}><X/></button></div>;
}
...
function SimpleLine({values,dates}:{values:number[];dates?:string[]}){
