...
import {VexumDialog,VexumPageSkeleton,requestVexumPrompt,pushVexumToast} from './VexumUi';
import {
...
export default function VexumLife({section,onSectionChange}:{section?:string;onSectionChange?:(section:string)=>void}={}){
  ...
  useEffect(()=>{if(!lifeSections.includes(tab))setTab(lifeSections[0]||'Today')},[platform.lifeSections.join('|'),tab]);
  ...
  const scheduleTask=async(task:LifeTask)=>{
    const date=await requestVexumPrompt({title:'Schedule task',description:task.title,label:'Date',defaultValue:task.dueDate||today,inputType:'date',confirmLabel:'Next'});if(!date)return;
    const time=await requestVexumPrompt({title:'Start time',description:task.title,label:'Time',defaultValue:task.dueTime||'09:00',inputType:'time',confirmLabel:'Next'})||'09:00';
    const durationRaw=await requestVexumPrompt({title:'Duration',description:'How long should this task block be?',label:'Minutes',defaultValue:String(task.durationMinutes||30),inputType:'number',confirmLabel:'Schedule'});if(durationRaw===null)return;
    const duration=Math.max(1,Number(durationRaw)||30);
...
function TodayView({life,tasks,events,habits,workouts,bills,preorders,sellOrders,fitnessEnabled,overdue,onCompleteTask,onToggleHabit,onOpenTasks,onOpenCalendar,onOpenFitness,onOpenFinancial,onOpenWishlist}:{
  life:LifeData;tasks:LifeTask[];events:LifeEvent[];habits:LifeHabit[];workouts:WorkoutPlan[];bills:Array<{id:string;name:string;amount:number;nextDueDate?:string}>;preorders:any[];sellOrders:SellOrder[];fitnessEnabled:boolean;overdue:LifeTask[];
...
function TaskInspector({task,life,onUpdate,onSchedule,onDelete}:{task:LifeTask;life:LifeData;onUpdate:(t:LifeTask)=>void;onSchedule:(t:LifeTask)=>void;onDelete:()=>void}){
  ...
  const addSubtask=async()=>{const title=await requestVexumPrompt({title:'Add subtask',label:'Subtask',placeholder:'What needs to be done?',confirmLabel:'Add'});if(!title?.trim())return;patch({subtasks:[...task.subtasks,{id:newLifeId('subtask'),title:title.trim(),completed:false}]})};
...
function FitnessView({life,range,setRange,onAddPlan,onPersist}:{life:LifeData;range:'7'|'30';setRange:(v:'7'|'30')=>void;onAddPlan:()=>void;onPersist:(d:LifeData)=>void}){
  ...
  return <div className="vxl-fitness"><div className="vxl-section-head"><div><h2>Fitness</h2><p>Training activity, volume, consistency, and progression. The muscle map represents training load—not biological growth.</p></div><button onClick={onAddPlan}><Plus/>Workout Plan</button></div>
    ...
    <div className="vxl-fitness-grid">
      ...
      <section className="vx-panel vxl-prs"><header><h3>Personal Records</h3></header>{prs.slice(0,8).map(pr=><div key={pr.name}><Trophy/><span><strong>{pr.name}</strong><small>{pr.muscle}</small></span><b>{pr.weight} lb</b></div>)}{!prs.length?<Empty compact text="PRs appear after logged weighted sets."/>:null}</section>
    </div>
  </div>;
}
...
function WorkoutModal({onClose,onSave}:{onClose:()=>void;onSave:(p:WorkoutPlan)=>void}){
  ...
  const addExercise=async()=>{const exName=await requestVexumPrompt({title:'Add exercise',label:'Exercise name',placeholder:'Lat pulldown',confirmLabel:'Next'});if(!exName?.trim())return;const muscle=await requestVexumPrompt({title:'Primary muscle group',label:'Muscle group',defaultValue:'Back',placeholder:'Chest, Back, Shoulders…',confirmLabel:'Next'})||'Other';const setsRaw=await requestVexumPrompt({title:'Sets',label:'Sets',defaultValue:'3',inputType:'number',confirmLabel:'Next'});if(setsRaw===null)return;const repsRaw=await requestVexumPrompt({title:'Reps',label:'Reps',defaultValue:'10',inputType:'number',confirmLabel:'Next'});if(repsRaw===null)return;const weightRaw=await requestVexumPrompt({title:'Default weight',description:'Optional. Leave blank if you do not want a default.',label:'Weight',defaultValue:'',inputType:'number',confirmLabel:'Add Exercise'});if(weightRaw===null)return;const sets=Math.max(1,Number(setsRaw)||3),reps=Math.max(1,Number(repsRaw)||10),weight=Number(weightRaw||0)||undefined;setExercises(cur=>[...cur,{id:newLifeId('exercise'),name:exName.trim(),muscleGroup:muscle,sets,reps,weight,notes:''}])};
...
function Modal({title,subtitle,onClose,children}:{title:string;subtitle:string;onClose:()=>void;children:ReactNode}){return <VexumDialog open onClose={onClose} title={title} description={subtitle} size="lg" className="vxl-shared-dialog">{children}</VexumDialog>}
