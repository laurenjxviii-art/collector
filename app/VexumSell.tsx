...
function Metric({label,value,sub,tone='muted',trend,trendLabel='Recorded history'}:{label:string;value:string;sub:string;tone?:'muted'|'green'|'red'|'orange';trend?:number[];trendLabel?:string}){
  return <section className="vxsel-metric"><span>{label}</span><strong>{value}</strong><small className={'tone-'+tone}>{sub}</small>{trend!==undefined?<VexumMetricTrend values={trend} label={trendLabel} negative={tone==='red'}/>:null}</section>;
}
  ...
  return <div className="vxsel-page">
    ...
    {session&&tab==='Overview'?<>
      <div className="vxsel-metrics">
        ...
        <Metric trend={[0,...monthOrders.toSorted((a,b)=>(a.completed_at||a.updated_at).localeCompare(b.completed_at||b.updated_at)).map((_,index,rows)=>rows.slice(0,index+1).reduce((sum,order)=>{const item=orderItemByOrder.get(order.id);return sum+order.net_proceeds-(item?.cost_basis_snapshot||0)*(item?.quantity||1)},0))]} trendLabel="Cumulative monthly profit" label="Net Profit This Month" value={(monthProfit>=0?'+':'')+money(monthProfit)} sub={money(monthNet)+' net proceeds'} tone={monthProfit>=0?'green':'red'}/>
        <Metric trend={completedOrders.toSorted((a,b)=>(a.completed_at||a.updated_at).localeCompare(b.completed_at||b.updated_at)).map(order=>{const item=orderItemByOrder.get(order.id);return order.item_subtotal?(order.net_proceeds-(item?.cost_basis_snapshot||0)*(item?.quantity||1))/order.item_subtotal*100:0})} trendLabel="Margin by completed sale" label="Average Margin" value={completedOrders.length?(averageMargin>=0?'+':'')+pct(averageMargin):'—'} sub="Completed sales only"/>
